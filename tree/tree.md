...
- 우선순위 큐; Priority Queue를 만들 때 자주 쓰이는 자료구조
    ...
    - 자료구조에 따른 우선순위 큐 시간 복잡도(삽입/삭제)
        ...
        - heap : O(log N) / O(log N)
            ...
            - 그래서 heap의 시간 복잡도는 트리의 높이에 비례함.

### BigO
- 시간복잡도 기준
- 생성: O(N)
- 정렬: O(log N) (평균, 최악 전부 그런 듯)
- 참고로, 병합정렬(merge sort)이 같은 시간복잡도를 갖는데, heap sort가 메모리를 덜 써서 더 낫다는 듯
...
- 배열에서 이진트리 노드 인덱스 찾기
    ...
    - 1-based 배열인 경우 (`example.go`의 MinHeap이 여기에 해당됨.)
        - 왼쪽 자식 노드의 인덱스: parent_idx * 2
        - 오른쪽 자식 노드의 인덱스: parent_idx * 2 + 1
        - 부모 노드의 인덱스: child_idx/2 
- 구현을 쉽게 하기 위해 0번 인덱스를 안 쓰는 경우(1-based 배열이랑 같음.)도 있다고 함.
- heap 만들 때 필요한 메서드
    - `down_heap`
